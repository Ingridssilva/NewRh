...
-- Também registre o usuário responsável pela aprovação final como ROLE_OWNER no banco:
-- (rode só se essa conta ainda não existir)
-- INSERT INTO users (username, email, role, is_active)
-- VALUES ('owner', 'owner@empresaexemplo.com.br', 'ROLE_OWNER', true)
-- ON CONFLICT (username) DO UPDATE SET role = 'ROLE_OWNER', is_active = true;

-- Ou se a conta já existe, só atualize o role:
-- UPDATE users SET role = 'ROLE_OWNER' WHERE username = 'owner';
