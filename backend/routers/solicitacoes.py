...
Fluxo: Gestor solicita → Diretoria aprova (e-mail/portal) → Vaga criada automaticamente
Notificações: Diretoria recebe e-mail com CC para TI
...
import audit
from email_service import notify_solicitacao_aprovador, notify_resultado_solicitacao, notify_solicitacao_gestor

bp = Blueprint("solicitacoes", __name__, url_prefix="/api/solicitacoes")

OWNER_EMAIL = "owner@empresaexemplo.com.br"
...
def criar_solicitacao():
    ...
    try:
        ...
        # Notifica diretoria (CC: TI)
        try:
            notify_solicitacao_aprovador(sol, OWNER_EMAIL, _base_url())
        except Exception as e:
            print(f"[EMAIL] Erro ao notificar diretoria: {e}")
...
def revisar_portal():
    ...
    try:
        ...
        if sol.status != "PENDENTE":
            return jsonify({"message": "Solicitação já decidida"}), 409
        return _processar_decisao(db, sol, decision, motivo, "diretoria")
    finally:
        db.close()
...
def revisar_por_email():
    ...
    if decision in ("APROVADA", "REJEITADA"):
        ...
        try:
            ...
            resp = _processar_decisao(db, sol, decision, None, "diretoria")
...
def _notificar_candidatos_nova_vaga(sol, db):
    ...
    try:
        ...
        for c in candidatos:
            ...
            subject = f"🎯 Nova vaga disponível: {sol.position} — Empresa Exemplo"
            content_html = (
                ...
                f'Empresa Exemplo · Portal de Carreiras</p>'
            )
...
<html lang="pt-BR">
<head><meta charset="UTF-8"><title>Resultado — Empresa Exemplo</title>
...
<h1>Empresa Exemplo — Portal de Vagas</h1>
<p>{mensagem}</p>
