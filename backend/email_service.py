"""
Serviço de e-mail — Empresa Exemplo
...
SENDER_EMAIL  = os.getenv("MS_SENDER_EMAIL",  "rh@empresaexemplo.com.br")
    ...
    <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
      <tr>
        <td style="background:linear-gradient(135deg,#141820,#1C2230);
          ...
          <div style="font-size:28px;font-weight:900;color:#fff;letter-spacing:-1px;">⚡ Empresa Exemplo</div>
      ...
      <tr>
        <td style="background:#0F1318;border:1px solid rgba(255,255,255,0.05);
          ...
          <p style="margin:0;font-size:12px;color:rgba(255,255,255,0.25);line-height:1.6;">
            Empresa Exemplo Ltda · Cidade, Estado<br>
...
<p style="color:#A8A8B8;font-size:15px;line-height:1.7;margin:0;">
  ...
  Encorajamos você a acompanhar novas oportunidades no nosso portal. 💪
        ...
        "PENDING": {
            "subject": f"Candidatura recebida — {cargo} | Empresa Exemplo",
...
def build_invite_email(username: str, email: str, token: str, base_url: str) -> tuple[str, str]:
    ...
    subject = "⚡ Seu acesso ao Portal de Carreiras — Empresa Exemplo"
    content = f"""
...
<p style="color:#A8A8B8;font-size:15px;line-height:1.7;margin:0 0 14px;">
  ...
  <strong style="color:#fff;">Portal de Carreiras da Empresa Exemplo</strong> como gestor.
</p>
<p style="color:#A8A8B8;font-size:15px;line-height:1.7;margin:0 0 14px;">
...
def build_reset_email(username: str, token: str, base_url: str) -> tuple[str, str]:
    ...
    subject = "🔑 Redefinição de senha — Portal Empresa Exemplo"
...
<p style="color:#A8A8B8;font-size:15px;line-height:1.7;margin:0 0 14px;">
  ...
  <strong style="color:#fff;">Portal de Carreiras da Empresa Exemplo</strong>.
...
<p style="color:#A8A8B8;font-size:14px;line-height:1.7;margin:16px 0 0;">
  ...
  <a href="#" style="color:{BRAND_COLOR};text-decoration:none;font-weight:600;">
    Portal Empresa Exemplo → Acompanhar candidatura
  </a>
</p>
...
<p style="color:#5A6478;font-size:13px;line-height:1.6;margin:0;">
  ...
  Obrigado pelo interesse em fazer parte do time Empresa Exemplo! ⚡
</p>"""
    ...
    <div style="font-family:'Segoe UI',Arial,sans-serif;background:#0B0F14;padding:40px 0;">
      <div style="max-width:560px;margin:0 auto;background:#141820;border-radius:16px;overflow:hidden;border:1px solid rgba(255,255,255,0.07);">
        <div style="background:linear-gradient(135deg,#FF6A00,#E55A00);padding:28px 32px;">
          <h1 style="font-size:22px;font-weight:900;color:#fff;margin:0;">⚡ Empresa Exemplo</h1>
        ...
        <div style="padding:16px 32px;border-top:1px solid rgba(255,255,255,.06);text-align:center;">
          <p style="color:#5A6478;font-size:12px;margin:0;">© 2026 Empresa Exemplo · Portal de Admissão</p>
        </div>
      </div>
    </div>"""
    return subject, html
...
EMAIL_TI    = "ti@empresaexemplo.com.br"
EMAIL_RH    = "rh@empresaexemplo.com.br"
EMAIL_DP    = "DP@empresaexemplo.com.br"
EMAIL_SESMT = "SESMT@empresaexemplo.com.br"
EMAIL_RH_CC = [                                    # CC nas notificações ao gestor
    "RH@empresaexemplo.com.br",
    "DP@empresaexemplo.com.br",
    "ti@empresaexemplo.com.br",
    "rh@empresaexemplo.com.br",                    # ← adicionado
]
...
def notify_solicitacao_aprovador(sol, aprovador_email: str, base_url: str):
    """Envia pedido de aprovação à diretoria (TO) e cópia informativa ao TI (CC)."""
...
<p style="color:#5A6478;font-size:12px;text-align:center;margin:0;">
  ℹ️ TI foi copiado neste e-mail apenas para acompanhamento.</p>"""
    ...
    html = _base_template(subject, content)
    send_email(aprovador_email, subject, html, cc=["ti@empresaexemplo.com.br"])
...
def notify_resultado_solicitacao(sol, base_url: str):
    ...
    mensagem = (
        ...
        + ("A vaga já está publicada no portal." if aprovada
           else "Entre em contato com a diretoria para mais informações.")
    )
...
<p style="color:#5A6478;font-size:12px;text-align:center;margin:0;">
  Este é um aviso automático do sistema NewRH · Empresa Exemplo
</p>"""
